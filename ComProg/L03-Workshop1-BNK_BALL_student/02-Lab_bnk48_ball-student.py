...
ball2_radius = 50
ball3_radius = 60
file_name = os.listdir('source/BNK48')
file_name.remove('Jane_cc.png')
...
pg.mixer.init()
pg.mixer.music.load("D:\Code\CU\ComProg\Workshop1-BNK_BALL_student\source\sound.mp3")
pg.mixer.music.play(-1, 0.0)

# ใช้คำสั่ง soundeffect.play() เพื่อเล่นเสียง effect ตอนลูกบอลชนกัน
soundeffect = pg.mixer.Sound("D:\Code\CU\ComProg\Workshop1-BNK_BALL_student\source\effect.wav")
...
ball2_img = pg.image.load("source/BNK48/Satchan_cc.png").convert_alpha()
ball2_img = pg.transform.scale(ball2_img, (100, 100))
ball2_rect = ball1_img.get_rect(center=(250, 120))

# Member 3 [size : (120 , 120) , center : (800 , 400)]
ball3_img = pg.image.load("source/BNK48/Pun_cc.png").convert_alpha()
ball3_img = pg.transform.scale(ball3_img, (120, 120))
ball3_rect = ball1_img.get_rect(center=(800, 400))
...
while running:
    ...
    if running:
        ...
        text_surface = font.render("6432133721", True, white)  # กำหนด Text และ สี
        text_rect = text_surface.get_rect()  # แปลง Surface เป็น object
        ...
        distance_ball1_ball2 = (ball1_rect.centerx-ball2_rect.centerx)**2 + (ball1_rect.centery-ball2_rect.centery)**2
        distance_ball2_ball3 = (ball2_rect.centerx-ball3_rect.centerx)**2 + (ball2_rect.centery-ball3_rect.centery)**2
        distance_ball1_ball3 = (ball1_rect.centerx-ball3_rect.centerx)**2 + (ball1_rect.centery-ball3_rect.centery)**2

        if distance_ball1_ball2 <= (ball1_radius + ball2_radius)**2:
            ball1_speed[0] = -ball1_speed[0]
            ball1_speed[1] = -ball1_speed[1]
            ball2_speed[0] = -ball2_speed[0]
            ball2_speed[1] = -ball2_speed[1]
            ball1_img = pg.image.load("source/BNK48/" + rd.choice(file_name)).convert_alpha()
            ball1_img = pg.transform.scale(ball1_img, (150, 150))
            ball1_rect = ball1_img.get_rect(center=ball1_rect.center)

            ball2_img = pg.image.load("source/BNK48/" + rd.choice(file_name)).convert_alpha()
            ball2_img = pg.transform.scale(ball2_img, (100, 100))
            ball2_rect = ball2_img.get_rect(center=ball2_rect.center)

            soundeffect.play()
            cnt += 1
        if distance_ball2_ball3 <= (ball2_radius + ball3_radius)**2:
            ball3_speed[0] = -ball3_speed[0]
            ball3_speed[1] = -ball3_speed[1]
            ball2_speed[0] = -ball2_speed[0]
            ball2_speed[1] = -ball2_speed[1]

            ball3_img = pg.image.load("source/BNK48/" + rd.choice(file_name)).convert_alpha()
            ball3_img = pg.transform.scale(ball3_img, (120, 120))
            ball3_rect = ball3_img.get_rect(center=ball3_rect.center)
            ...
            soundeffect.play()
            cnt += 1
        if distance_ball1_ball3 <= (ball1_radius + ball3_radius)**2:
            ball1_speed[0] = -ball1_speed[0]
            ball1_speed[1] = -ball1_speed[1]
            ball3_speed[0] = -ball3_speed[0]
            ball3_speed[1] = -ball3_speed[1]
        ...
        # Addition: แสดงจำนวนครั้งที่เกิดการชน
        font = pg.font.Font(font_name, 50)
        text_surface = font.render(str(cnt), True, white)  # กำหนด Text และ สี
        text_rect = text_surface.get_rect()  # แปลง Surface เป็น object
